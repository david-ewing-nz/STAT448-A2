...
```{r scroll-box-function, include=FALSE}
...
residen_file <- "Residen.RData"
residen_path <- path(data_folder,residen_file) # because we dont use the same folder structure
load(residen_path)
Data <- Residen
...
parkinsons_path <- path(data_folder,parkinsons_file) # because we dont use the same folder structure
...
weather_path <- path(data_folder,weather_file) # because we dont use the same folder structure
weather_data <- read.csv(weather_path, header = TRUE)[-1]
